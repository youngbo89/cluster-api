...
// UpdateEtcdConditionsAsUnknown is responsible for updating machine as uknown
// It should be called instead of Workload.UpdateEtcdConditions when Workload instance cannot be created
func UpdateEtcdConditionsAsUnknown(ctx context.Context, controlPlane *ControlPlane) {
	if controlPlane.IsEtcdManaged() {
...
func (w *Workload) UpdateStaticPodConditions(ctx context.Context, controlPlane *ControlPlane) {
	allMachinePodConditions := getAllMachineStaticPodConditions(controlPlane)
...
// UpdateStaticPodConditionsAsUnknown is responsible for updating machine conditions as unknown
// It should be called instead of Workload.UpdateStaticPodConditions when Workload instance cannot be created
func UpdateStaticPodConditionsAsUnknown(ctx context.Context, controlPlane *ControlPlane, cause error) {
	updateStaticPodConditionsAsUnknown(ctx, controlPlane, getAllMachineStaticPodConditions(controlPlane), cause)
}
...
func getAllMachineStaticPodConditions(controlPlane *ControlPlane) []clusterv1.ConditionType {
	...
	if controlPlane.IsEtcdManaged() {
		return append(allMachinePodConditions, controlplanev1.MachineEtcdPodHealthyCondition)
	}
